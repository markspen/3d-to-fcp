...
## 1.0(6) — Unlocked grant panel (2026-07-05, commit e7c5f09 on main)
- **3rd 1.0(5) rejection (Jul 5, MacBook Pro 14") decoded:** the canned 2.4.5(i)/movies-entitlement text is still stale (binary proven clean), but a NEW human-written sentence appeared: *"have the user choose the default location using standard Open/Save dialogs instead of pre-setting it."* The reviewer read the **locked-down panel** (only Movies selectable, everything else greyed) as PRE-SETTING the location — functionally the movies entitlement by another route. Lesson: **a "user-selected" grant must be a GENUINE choice; locking the panel to one folder defeats the point in App Review's eyes. Guide, don't enforce.**
